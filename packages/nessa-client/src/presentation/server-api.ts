import type { HealthResult, PingResult } from "../protocol/index.js"
import { Method } from "../generated/catalog.js"
import { assertHealthResult, assertPingResult } from "../protocol/validate.js"
import type { WireSession } from "../transport/wire-session.js"

/** Typed `server.*` RPC namespace on a connected client. */
export type ServerApi = {
  health: () => Promise<HealthResult>
  ping: (nonce: string) => Promise<PingResult>
}
...
export function createServerApi(session: WireSession): ServerApi {
  return {
    ...
    ping: async (nonce: string) =>
      assertPingResult(await session.request(Method.ServerPing, { nonce })),
  }
}
