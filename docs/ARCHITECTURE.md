...
| `composition.rs` | The composition root: the one place the host's outside things are constructed — settings, shortcuts, the surface credential, the agent credential writer and audit, the gateway, the release source — and the bundle every command and menu is given. Nothing below it reaches back for a dependency. |
