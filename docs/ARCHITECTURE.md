...
| `application/attachment-notice.ts`, `ui/attachment-notification.tsx` | Everything the composer says about the draft's files. One typed refusal per way the panel turns something away, one ranked notice over both those and the draft's own upload state, and one action at most: retrying the uploads worth retrying, or opening the file picker only where choosing again could end differently. The words for a failed upload are the conversation's. Rendered through `AgentNotification`, like every other notice in this pane; no notice is red text beside the composer. |
...
| `ui/use-file-attachments.ts` | Remote pending previews, originating conversation, viewer state, and the last refusal — which it holds as a typed reason, never as a sentence. Local files use synchronous object URLs. Uploading is not its job. |
