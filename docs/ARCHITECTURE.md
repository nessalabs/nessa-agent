...
| `application/ports.ts` | `ConversationGateway` for local draft and tab operations; `ConversationEffects` for what the panel may ask the product to do, including staging an image's bytes; the typed `AttachmentStagingError`, `SubmissionRefusedError` (a message the gateway did not take), and `ControlFailedError` (a control it answered with a reason, which does not say the control was not applied). |
...
| `application/usecases/control-failure.ts` | What a failed conversation control says, chosen by the same typed reason a refused message is. A control has no draft to hand back, so the news is not the same; where it has nothing of its own to add, the client's sentence — which names the command — is shown instead. |
