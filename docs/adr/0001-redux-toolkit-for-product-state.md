...
going through React. The open conversation tabs are the first piece of that
state. Host concerns (frost, resize, the window size) are subscriptions to the
...
open conversation tabs. It is a projection: named actions call
`ConversationGateway` and store the tabs. `store.dispatch` is the agent
...
- **Keep `useState`.** Cheap today, and the tabs are still few. It cannot
...
and the panel updates. Tests of the tabs do the same, with no renderer.
