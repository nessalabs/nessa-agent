...
| 10 | [202 — Versioned local datasets](todo/202-versioned-local-datasets.md) | Proposed, draft: every local store classed critical, record-scoped or degradable, one version per dataset shape, forward-only migrations shipped with the build, and a typed `datasetRefused` startup refusal. Awaiting the owner's answers before any implementation |
