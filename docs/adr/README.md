...
| 11 | [221 — Startup refusals are handled or said plainly](todo/221-startup-refusals.md) | Accepted. Implemented, in review: setup opens a window instead of aborting, the one shared-read repair, named startup steps and the panel's startup notice, named retirement refusals and the macOS stop of a gateway whose data is gone. Remaining: acting on `data_missing` in the systemd adapter |
