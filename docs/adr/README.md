...
| 10 | [202 — Versioned local datasets](todo/202-versioned-local-datasets.md) | Accepted. Implemented, in review: conversation metadata that is at another version or is not a database refuses the gateway as `datasetRefused`, not retried, recorded, and said by the host. Remaining: a typed "unreadable" open for the browser-session journal, then the same refusal; migrations once Nessa leaves alpha |
