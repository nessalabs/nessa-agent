...
- `scripts/desktop/prepare.mjs` selects runtime preparation on macOS and Linux;
  ...
  service. Linux builds bundle that prepared resource into a `.deb` and an
  AppImage; `verify-linux-bundle.mjs` checks the runtime inside each, as
  ...
  target publishes and its updater key: one per macOS architecture, one per
  Linux package format. Windows has no preparation.
