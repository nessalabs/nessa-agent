...
- `settings.onboarding.completed` records that first-run setup finished. `src-tauri/src/main.rs` opens the setup window only when it is false; `panel::complete_onboarding` (invoked through `src/host/window.ts`'s `recordSetupComplete`, from `src/onboarding/ui/setup-gate.tsx` once the handoff to the panel has landed) writes it. `src/onboarding/application/setup-completion.ts` holds the rule: only a finished setup — not a dismissed one — and only after the panel actually came up or there was no second window to hand over to, so a failed handoff leaves first run still to do. The debug-only tray item clears it through `panel::restart_onboarding`.
