...
- `src-tauri/src/gateway/infrastructure/linux/` owns the packaged Linux systemd user service. It refuses hosts without the account user manager, logind linger, the owned `UnitPath`, the root-owned non-symlink `/usr/bin/env` launcher, or pidfd support before admitting an intent, and it repeats the UID, desktop-injected XDG roots, source fingerprint, installed-unit snapshot, endpoint, manager owner, and path authority checks immediately before admission. It publishes a private immutable runtime and unit definition, then an identity-checked `default.target.wants` link, and confirms enablement through `GetUnitFileState`; interrupted definition and link transactions settle only the exact owned temporary entries. Start and stop always use non-replayable `fail` jobs: the returned manager owner, unit, operation, object path, and numeric job ID are journaled before waiting for the matching `JobRemoved` signal. The command result and a fresh physical observation are separate facts: start succeeds only for the exact active target with its native manager, invocation, PID, definition, endpoint, and link evidence, while stop succeeds only after no active incarnation remains, regardless of whether the job acknowledgement was accepted, rejected, or lost. Replacement keeps the old link and runtime, journals the retirement request identity, rechecks its exact definition and manager owner, requires the gateway's durable request/result acknowledgement tuple, then stops the exact old unit before publishing the next definition. Quit opens a pidfd for the corroborated endpoint and systemd PID, rechecks the exact definition and manager authority, and consumes the application proof immediately before `pidfd_send_signal`. Recovery records native systemd observations, settles every authenticated crash prefix without replaying an unacknowledged D-Bus enqueue, and preserves ambiguous names and identities for diagnosis.
