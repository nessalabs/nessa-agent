...
- `src-tauri/src/gateway/infrastructure/linux/` owns the packaged Linux systemd user service. It refuses hosts without the account user manager, logind linger, the owned `UnitPath`, or pidfd support before admitting an intent. It publishes a private immutable runtime and unit definition, then an anchored no-replace `default.target.wants` link, and confirms enablement through `GetUnitFileState`. Start and stop always use non-replayable `fail` jobs: the returned manager owner, unit, operation, object path, and numeric job ID are journaled before waiting for the matching `JobRemoved` signal. Readiness requires agreement across the unit object, invocation ID, main PID, definition, empty manager environment, `ExecStartEx`, managed endpoint publication, and wants link. Replacement keeps the old link and runtime, journals the retirement request identity, requires the gateway's durable request/result acknowledgement tuple, then stops the exact old unit before publishing the next definition. Quit opens a pidfd for the corroborated endpoint and systemd PID and consumes the application proof immediately before `pidfd_send_signal`. Recovery observes every crash prefix and never blindly replays an unacknowledged D-Bus enqueue.
