 ...
 *
 * The file is base64 of a minisign block. An empty or truncated one still
 * parses as a manifest field and fails only on a user's machine, at install,
 * as a rejected signature — so it is checked here, where it can still stop a
 * release. This says the signature is well formed, not that it verifies: the
 * key pairing gate (`src-tauri/tests/updater_key_pairing.rs`) is what proves
 * the shipped public key can verify it, and it runs before any of this. */
