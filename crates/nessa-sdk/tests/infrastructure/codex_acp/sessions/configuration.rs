...
fn a_new_session_is_checked_for_a_model_codex_would_accept_not_one_it_has_selected() {
    ...
    let offered = options(model_option("codex-default", &["codex-default", "exact"]), json!({}));
    ...
    assert_eq!(
        verify_config(&options(model_option("exact", &["codex-default"]), json!({})), "exact", None),
        Ok(())
    );
}
...
fn a_configured_session_must_read_back_both_selections_exactly() {
    assert_eq!(
        verify_config(
            &options(model_option("exact", &["exact"]), json!({"id":"mode","currentValue":MODE})),
            "exact",
            Some(MODE)
        ),
        Ok(())
...
fn a_response_without_readable_options_is_never_read_as_agreement() {
    for result in [
        ...
        assert!(verify_config(&result, "exact", Some(MODE)).is_err(), "{result}");
    }
}
