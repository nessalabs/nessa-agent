...
use crate::domain::agent_execution::{executions::ExecutionId, prompts::PromptText};
...
pub struct ExecutionRequest {
    ...
    /// New user text, at most [`Self::MAX_MESSAGE_BYTES`] UTF-8 bytes.
    /// Previously submitted messages are not replayed here.
    pub user_message: PromptText,
...
impl ExecutionRequest {
    /// Maximum new user-message payload: 4 MiB of actual UTF-8 text, independent
    ...
    pub(crate) fn validate_message_size(&self) -> Result<(), AgentError> {
        if self.user_message.as_str().len() > Self::MAX_MESSAGE_BYTES {
