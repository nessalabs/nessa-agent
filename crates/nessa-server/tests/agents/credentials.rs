...
/// Waiting for a vendor's own tool when it will not finish, with an ordinary
/// long-running command standing in for one. The real keychains and command-line
/// tools are the host's to answer and are never driven here; what is under test
/// is the waiting every agent's probe shares.
#[cfg(unix)]
mod when_a_tool_will_not_finish {
    ...
    /// A process that would outlive any request, started the way a sign-in
    /// lookup is started.
