...
impl AgentId {
    ...
    /// agent at all.
    ///
    /// Every configured agent uses a person's vendor account. Packaged OpenCode
    /// starts on a metered Zen model, so a supported saved API key is a launch
    /// precondition rather than an optional upgrade from a free default.
