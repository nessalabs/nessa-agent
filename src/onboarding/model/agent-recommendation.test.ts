 ...
 *
 * OpenCode, which is the one Nessa actually pins a release of — not a stand-in
 * for it. The module is still deliberately not written around the name, and
 * every case below would read the same for a fourth agent; what using the real
 * one buys is that the offer path is exercised with the data it will carry.
 */
const INSTALLABLE: readonly AgentId[] = ["opencode"]
...
/** Every listed agent reporting that it is not on the machine. */
const NOTHING_INSTALLED: AgentReadinessReport = Object.fromEntries(
  AGENT_CHOICES.map((choice) => [choice.id, "not-installed"]),
)
...
describe("recommendAgent", () => {
  ...
  it("lets the person choose when an agent can start", () => {
    const state = asked({ claude: "ready", opencode: "not-installed" })
  ...
  it("lets the person choose when the installable agent is the ready one", () => {
    ...
    const state = asked({ opencode: "ready" })
    expect(recommendAgent(state, INSTALLABLE)).toEqual({ kind: "choose" })
  })
  ...
  it("offers the installable agent when nothing can start", () => {
    ...
    expect(recommendAgent(asked(NOTHING_INSTALLED), INSTALLABLE)).toEqual({
      kind: "install",
      agent: "opencode",
    })
  })
  ...
  it("does not offer to install an agent that is installed and signed out", () => {
    ...
    // missing install.
    const state = asked({ opencode: "needs-authentication" })
    expect(recommendAgent(state, INSTALLABLE)).toEqual({ kind: "nothing" })
  })

  it("offers nothing when Nessa can install nothing", () => {
    expect(recommendAgent(asked(NOTHING_INSTALLED), [])).toEqual({ kind: "nothing" })
  })

  it("offers only the listed agents this build has an adapter for", () => {
    // Every entry is supported today, so this reads as "all three are offered".
    // It is written against the listing rather than against three names because
    // the day a fourth agent is listed ahead of its adapter, this is the case
    // that has to fail: a pinned release for an agent Nessa cannot drive must
    // not produce an offer that ends in a download and a row that still cannot
    // be selected.
    for (const choice of AGENT_CHOICES) {
      expect(
        recommendAgent(asked(NOTHING_INSTALLED), [choice.id]),
        `${choice.id} is listed as supported: ${choice.supported}`,
      ).toEqual(
        choice.supported ? { kind: "install", agent: choice.id } : { kind: "nothing" },
      )
    }
  })
  ...
  it("offers nothing for an agent setup does not list at all", () => {
    ...
    const state = asked(NOTHING_INSTALLED)
  ...
  it("offers nothing on a readiness that is the absence of an answer", () => {
    ...
    for (const readiness of ["unknown", "not-supported"] as const) {
      expect(
        recommendAgent(asked({ opencode: readiness }), INSTALLABLE),
  ...
  it("offers nothing on a readiness this build has not heard of", () => {
    ...
    const state = asked({ opencode: "not-configured" as never })
    expect(recommendAgent(state, INSTALLABLE)).toEqual({ kind: "nothing" })
  })

  it("offers nothing for an agent nobody answered about", () => {
    // The defect this guards: a global "did anybody answer" check passes on
    // codex's answer, and the offer is then made about opencode, which nothing
    // was said about. Somebody whose opencode is installed and working — but
  ...
  it("skips an unofferable agent to reach one it can offer", () => {
    // Codex is on the machine and signed in; opencode is not here at all. The
    // offer is the one that a download would actually change.
    const state = asked({ codex: "needs-authentication", opencode: "not-installed" })
    const several = ["codex", "opencode"] as readonly AgentId[]
    expect(recommendAgent(state, several)).toEqual({ kind: "install", agent: "opencode" })
  })
})
