...
pub fn install_and_restart(app: &AppHandle) {
    ...
    match std::env::current_exe().map(|executable| replaceable(&executable)) {
        ...
        Ok(Replaceable::Loose) => {
            eprintln!(
                "[nessa] refusing to install an update over a build that is not installed. \
                 The updater replaces the directory this executable runs from, which here \
                 would be the build directory and everything in it. Install a packaged \
                 build, or exercise the download with scripts/desktop/updater-harness.mjs."
            );
            return;
        }
        Err(error) => {
            eprintln!("[nessa] refusing to install: cannot tell what is running ({error})");
            return;
        }
    }
